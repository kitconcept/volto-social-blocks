...
describe('Soundcloud', () => {
  describe('extractSoundCloudId', () => {
    ...
    it('fails if not the iframe', () => {
      expect(
        extractSoundCloudId(
          '<iframe width="100%" height="300" scrolling="no" frameborder="no" allow="autoplay" src="https://plone.org"></iframe>',
        ),
      ).toBe('');
    });
  });
  describe('isValidSoundcloudId', () => {
    it('validates track id', () => {
      expect(
        isValidSoundcloudId(
          'https://api.soundcloud.com/tracks/472694601&color=#ff5500&auto_play=false&hide_related=false&show_comments=true&show_user=true&show_reposts=false&show_teaser=true&visual=true',
        ),
      ).toBe(true);
    });
